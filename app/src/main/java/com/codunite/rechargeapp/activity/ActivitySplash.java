...
public class ActivitySplash extends AppCompatActivity implements View.OnClickListener, WebServiceListener {
    ...
    private void StartApp() {
        ...
        new NoInternetScreen(svContext, root, ActivitySplash.this);
...
//                PreferenceConnector.writeString(svContext, PreferenceConnector.CASHBACKINCOME, user_detail_obj.getString("total_cashback_income"));
                ...
                FragHomeDashBoard.lstDashBoardAffiliate = new ArrayList<>();
                JSONArray affiliate_data = user_detail_obj.getJSONArray("affiliateList");
                for (int i = 0; i < affiliate_data.length(); i++) {
                    JSONObject data_obj = affiliate_data.getJSONObject(i);
                    String strName = data_obj.getString("title");
                    String imgUrl = GlobalVariables.PRE_URL_MAIN + data_obj.getString("image");
                    String webUrl = data_obj.getString("link");
                    FragHomeDashBoard.lstDashBoardAffiliate.add(new DashboardAffilateModel(strName, imgUrl, webUrl));
                }
