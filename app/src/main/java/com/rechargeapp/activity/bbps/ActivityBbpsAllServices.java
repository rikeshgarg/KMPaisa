...
//        }
        if (response == 0) {
            ...
            boolean isWalletLoading = act.checkEWalletAndAddWallet(amount, "Service", svContext,lstEditext,strOperatorCode,edRechargeAmount.getText().toString().trim(),serviceId,customToast);
            if (!isWalletLoading) {
                RechargeProcess(svContext,lstEditext,strOperatorCode,edRechargeAmount.getText().toString().trim(),serviceId,customToast);
            }
        }
    }

    public void RechargeProcess(Context svContext,List<EditText> lstEditext,String strOperatorCode,String amount,String serviceId,ShowCustomToast customToast) {
        String paramOne = "", paramTwo = "", paramThree = "";
        String paramFour = "", paramFive = "", paramSix = "";
        String paramSeven = "", paramEight = "";
